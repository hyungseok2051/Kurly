...
try {
			...
			String query = "select * from hr.kurly_goods where goodscode = ?";
			...
			while (rs.next()) {
				...
				String goodsimgage = rs.getString("goodsimgage");
				...
				GoodsVO goodsVO = new GoodsVO(goodscode, goodsimgage, goodsname, goodsinfo, goodsprice, goodsdiscount,
	...
	public GoodsVO findimg(String _goodscode) {
		...
		try {
			conn = dataFactory.getConnection();
			String query = "select goodsimgage from kurly_goods where goodscode= ?";
			System.out.println(query);
			pstmt = conn.prepareStatement(query);
			...
			rs.next();
				String goodsimgage = rs.getString("goodsimgage");
				findimg = new GoodsVO(goodsimgage);
