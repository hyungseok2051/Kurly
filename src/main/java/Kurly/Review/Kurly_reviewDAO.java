...
public class Kurly_reviewDAO {
	private DataSource dataFactory;
	private Connection conn;
	private PreparedStatement pstmt;
	private ResultSet rs;

	public Kurly_reviewDAO() {
		try {
	...
	// 선택한 상세페이지에서 후기 들고오기
	public List<Kurly_reviewVO> selectAllReview(String gcode1) {
		List<Kurly_reviewVO> ReviewList = new ArrayList();
		try {
			conn = dataFactory.getConnection();
			String query = "select * from hr.kurly_review where goodscode = ?";
...
//	}
	// 후기 작성하기
	public int insertRview(Kurly_reviewVO reviewVO) {
	...
	//작성한 후기글 들고오기
	public Kurly_reviewVO findreview(String _reviewnum) {
		Kurly_reviewVO findreview = null; 
		try {
			conn = dataFactory.getConnection();
			...
			findreview = new Kurly_reviewVO(id, goodscode, reviewnum, reviewcontents, reviewimage, goodsname);
	...
	//후기글 수정하기
	public void updatereview(Kurly_reviewVO reviewVO) {
		try {
			conn = dataFactory.getConnection();
			String reviewcontents = reviewVO.getReviewcontents();
			String reviewimage = reviewVO.getReviewimage();
			String reviewnum = reviewVO.getReviewnum();
			String query="update kurly_review set reviewcontents=? where reviewnum= ?";
			pstmt = conn.prepareStatement(query);
			pstmt.setString(1, reviewcontents);
			//pstmt.setString(2,reviewimage);, reviewimage=?
