...
<c:set var="totGoods" value="${goodsMap.totGoods}"/>
<c:set var="section" value="${goodsMap.section}"/>
<c:set var="pageNum" value="${goodsMap.pageNum}"/>
<%
...
">
 ...
 <c:choose>
 	...
 	<c:when test="${!empty goodsList}"> 
 		 ...
 		 <c:forEach var="goods" items="${goodsList}"  varStatus="start">
 		 
						<!--1번물품-->
			<div class="newproductcontent" >  <div style="display:none">"${(pageNum-1)*9}"</div>
				<div class="realnewproductarea">
					<div class="realnewproduct">
						<!--goodsselect-->
						<a
							href="${contextPath}/good1/goodsdetail.do?goodscode=${goods.goodscode}&goodsprice=${goods.goodsprice}&goodstitle=${goods.goodstitle}">
							<img alt="" src="${contextPath}/${goods.goodsimage}"> </a>
						<div>
    	...
    	<div align="center">
		<c:if test="${totGoods != null }">
			<c:choose>
				<c:when test="${totGoods > 90}">
					<c:forEach var="page" begin="1" end="10" step="1">
						<c:if test="${section > 1 && page == 1}">
							<a href="${contextPath}/good1/goodslist.do?section=${section-1}&pageNum=${(section-1)*9+1}">prev</a>
						</c:if>
						<a href="${contextPath}/good1/goodslist.do?section=${section}&pageNum=${page}">${(section-1)*9+pageNum}</a>
						<c:if test="${page ==10}">
							<a href="${contextPath}/good1/goodslist.do?section=${section+1}&pageNum=${section*9+1}">next</a>
						</c:if>
					</c:forEach>
				</c:when>
				<c:when test="${totGoods == 90}">
				...
				<c:when test="${totGoods < 90}">
					<c:forEach var="page" begin="1"  end="${totGoods/9+1}" step="1">
						<c:choose>
							<c:when test="${page==pageNum}">
								<a class="check" href="${contextPath}/good1/goodslist.do?section=${section}&pageNum=${page}" >${page}</a>
							</c:when>
							<c:otherwise>
								<a class="check"  href="${contextPath}/good1/goodslist.do?section=${section}&pageNum=${page}">${page}</a>
							</c:otherwise>	
